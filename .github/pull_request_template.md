## Objetivo

Descreva a alteração e a necessidade atendida.

## Versão documentada

- Versão/build da aplicação:
- Commit ou fonte da evidência:

## Verificações

- [ ] O conteúdo corresponde à versão declarada da aplicação.
- [ ] O sumário em `manual-usuario/index.md` foi atualizado, se necessário.
- [ ] Links e imagens locais foram conferidos.
- [ ] O PDF foi gerado e revisado.
- [ ] Capturas não expõem dados pessoais ou sensíveis.
- [ ] O histórico em `09-controle-do-documento.md` foi atualizado, se aplicável.
- [ ] As pendências editoriais afetadas foram atualizadas.
- [ ] A alteração recebeu validação funcional ou de negócio, quando necessária.

## Evidências

Informe os cenários verificados e anexe ou referencie as evidências relevantes.
